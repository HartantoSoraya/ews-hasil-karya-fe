                    ...
                    <VRow>
                        <VCol cols="12" md="6">
                            <VTextField v-model="code" label="Kode" placeholder="Kode EWS Device"
                                :error-messages="error && error.code ? [error.code] : []" :disabled="loading"
                                :loading="loading" readonly />
                        </VCol>

                        <VCol cols="12" md="6">
                            <VTextField v-model="name" label="Nama" placeholder="Nama EWS Device"
                                :error-messages="error && error.name ? [error.name] : []" :disabled="loading"
                                :loading="loading" readonly />
                        </VCol>

                        <VCol cols="12" md="6">
                            <VTextField v-model="type" label="Tipe" placeholder="Tipe EWS Device"
                                :error-messages="error && error.type ? [error.type] : []" :disabled="loading"
                                :loading="loading" readonly />
                        </VCol>

                        <VCol cols="12" md="6">
                            <VTextField v-model="province" label="Provinsi" placeholder="Provinsi EWS Device"
                                :error-messages="error && error.province ? [error.province] : []" :disabled="loading"
                                :loading="loading" readonly />
                        </VCol>

                        <VCol cols="12" md="6">
                            <VTextField v-model="regency" label="Kabupaten/Kota" placeholder="Kabupaten/Kota EWS Device"
                                :error-messages="error && error.regency ? [error.regency] : []" :disabled="loading"
                                :loading="loading" readonly />
                        </VCol>

                        <VCol cols="12" md="6">
                            <VTextField v-model="district" label="Kecamatan" placeholder="Kecamatan EWS Device"
                                :error-messages="error && error.district ? [error.district] : []" :disabled="loading"
                                :loading="loading" readonly />
                        </VCol>
                        ...
                        <VCol cols="12" md="6">
                            <VTextField v-model="subdistrict" label="Kelurahan/Desa"
                                ...
                                :disabled="loading" :loading="loading" readonly />
                        </VCol>

                        <VCol cols="12" md="6">
                            <VTextField v-model="description" label="Deskripsi" placeholder="Deskripsi EWS Device"
                        ...
                        <VCol cols="12">
                            ...
                            <EasyDataTable :headers="addressHistoryHeaders" :items="address_histories"
                                :loading="loading" :no-data-text="loading ? 'Memuat data...' : 'Tidak ada data'"
                                :no-results-text="loading ? 'Memuat data...' : 'Tidak ada data'"
                                :loading-text="loading ? 'Memuat data...' : 'Tidak ada data'"
                                :error-text="error ? 'Terjadi kesalahan saat memuat data' : 'Tidak ada data'" show-index
                                class="data-table">
                            </EasyDataTable>
                        </VCol>
                    </VRow>
